...
from arena.serve.monitor.basic_stats import report_basic_stats, get_log_files
from arena.serve.monitor.clean_battle_data import clean_battle_data
from arena.serve.monitor.elo_analysis import report_elo_analysis_results
from arena.utils import build_logger, get_window_url_params_js
...
(Note: Only anonymous votes are considered in this leaderboard. Check the full leaderboard for both anonymous votes and open votes.)
...
def get_full_table(anony_arena_df, full_arena_df, model_table_df):
    ...
    for i in range(len(model_table_df)):
        ...
        row.append(model_table_df.iloc[i]["MMMU"])
        # Organization
...
def build_leaderboard_tab(elo_results_file, leaderboard_table_file, video_elo_results_file, video_leaderboard_table_file, show_plot=False):
    ...
    if leaderboard_table_file:
        ...
        if video_leaderboard_table_file:
        ...
        if not show_plot:
            ...
            """
                ...
                ### Pairwise Winrate (Model A > Model B)

                ![Pairwise Winrate](https://wildvision-bench.github.io/win_fraction_heatmap.png)
...
#                 plot_4 = gr.Plot(p4, show_label=False)
    ...
    from arena.serve.gradio_web_server import acknowledgment_md
