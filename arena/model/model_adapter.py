...
from arena.constants import CPU_ISA
from arena.conversation import Conversation, get_conv_template
...
def load_deepseekvl_pretrained_model(
    ...
    from arena.vlm_utils.deepseek_vl.models import VLChatProcessor, MultiModalityCausalLM
    from arena.vlm_utils.deepseek_vl.utils.io import load_pil_images
...
def load_yivl_pretrained_model(
    ...
    from arena.vlm_utils.yi_llava.mm_utils import (
        get_model_name_from_path,
        load_pretrained_model,
    )
    from arena.vlm_utils.yi_llava.model.constants import DEFAULT_IMAGE_TOKEN, IMAGE_TOKEN_INDEX, key_info
    ...
    **kwargs,
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            from arena.vlm_utils.llava.model.language_model.llava_llama import LlavaConfig
...
def get_generate_stream_function(model: torch.nn.Module, model_path: str, image_processor: Optional[CLIPImageProcessor] = None, tokenizer: Optional[AutoTokenizer] = None):
    ...
    if is_llavav15_stream:
        ...
        from arena.model.model_llava import generate_stream_llava_v16
        return generate_stream_llava_v16
    elif is_qwenvl_stream:
        from arena.model.model_qwenvl import generate_stream_qwenvl
        return generate_stream_qwenvl
    elif is_blip_stream:
        from arena.model.model_instructblip import generate_stream_instructblip
        return generate_stream_instructblip
    elif is_uform_stream:
        from arena.model.model_uform import generate_stream_uform
        return generate_stream_uform
    elif is_tinyllava_stream:
        from arena.model.model_tinyllava import generate_stream_tinyllava
        return generate_stream_tinyllava
    elif is_deepseekvl_stream:
        from arena.model.model_deepseekvl import generate_stream_deepseekvl
